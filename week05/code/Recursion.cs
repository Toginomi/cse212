...
public static class Recursion
{
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        // Base case: if we've already computed the result for s, return it
        if (remember.ContainsKey(s)) return remember[s];

        // Recursive case: sum of ways to climb from the last three steps
        decimal ways =
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // Base case: if no wildcards left
        if (index == -1)
        {
            results.Add(pattern);
            return;
        ...
        // Recurse on both smaller problems
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        if (maze.IsEnd(x, y)) results.Add(currPath.AsString()); // Use this to add your path to the results array keeping track of complete maze solutions when you find the solution.

        else
        {
            // Recursive case: check all four directions
            ...
            foreach (var (dx, dy) in directions)
            {
                ...
                // Check if move is valid and not revisited
